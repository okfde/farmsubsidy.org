...
from .views import home, countries, country, recipient_short, recipient, search
...
urlpatterns = [
    ...
    country_url(
        r"recipient/(?P<recipient_id>[^/]+)/$",
        recipient_short,
        name="recipient_short_view",
    ),
    country_url(
        r"recipient/(?P<recipient_id>[^/]+)/(?P<slug>(.*))/",
        recipient,
        name="recipient_view",
    ),
]
